...
static float get_rotation_vector(float fStartAngle, float fEndAngle);
static int push_command_in_actor_queue(unsigned int command, actor *act);
static void queue_emote(actor *act, emote_data *emote);
static void rotate_actor_and_horse_range(int id, int mul);
static void sanitize_cmd_queue(actor *act);
static void unfreeze_horse(int i);
static void unqueue_cmd(int i);
static void unqueue_emote(actor *act);
static float unwindAngle_Degrees(float fAngle);
...
void add_command_to_actor(int actor_id, unsigned char command)
{
	...
	else if (command == leave_combat || command == enter_combat
		|| command == die1 || command == die2)
	{
		int j = 0;

		// Strip the queue of attack messages
		for (k = 0; k < MAX_CMD_QUEUE; k++)
		{
			switch (act->que[k])
			{
	...
	if (act->attached_actor >= 0)
	{
		// If in aim mode, ignore turning and ranging related commands. We do it
		// manually in next_command()
		switch(command)
		{
			case enter_aim_mode:
			case leave_aim_mode:
	...
	if (command==enter_combat)
	{
		...
		if (j >= 0 && act->que[j] == leave_combat)
		{
			...
			if (act->attached_actor >= 0 && j2 >= 0
				&& actors_list[act->attached_actor]->que[j2] == wait_cmd)
			{
	...
	else if (k > MAX_CMD_QUEUE-2)
	{
		int i;
		int k;
		LOG_ERROR("Too many commands in the queue for actor %d (%s) => skip emotes!",
...
#endif
#ifdef	ANIMATION_SCALING
				CalModel_Update(actors_list[i]->calmodel,
					(time_diff * actors_list[i]->cur_anim.duration_scale) / 1000.0f);
#else	/* ANIMATION_SCALING */
				CalModel_Update(actors_list[i]->calmodel,
					(((cur_time-last_update)*actors_list[i]->cur_anim.duration_scale)/1000.0));
#endif	/* ANIMATION_SCALING */
...
void destroy_actor(int actor_id)
{
	...
	for (i = 0; i < max_actors; i++)
	{
		if (actors_list[i] //The timer thread doesn't free memory
			&& actors_list[i]->actor_id == actor_id)
		{
			LOCK_ACTORS_LISTS();
			...
			if (attached_actor >= 0)
			{
				...
				if (attached_actor == max_actors-1)
				{
					...
					actors_list[max_actors] = NULL;
					if (actors_list[attached_actor]
						&& actors_list[attached_actor]->attached_actor >= 0)
...
void get_actor_damage(int actor_id, int damage)
{
	...
	if (act)
	{
		...
		{
			if (strcmp(act->actor_name, "Gargoyle")
				&& strcmp(act->actor_name, "Skeleton")
				&& strcmp(act->actor_name, "Phantom Warrior")) // Ideally, we'd also check to see if it was a player or not, but since this is just cosmetic...
			{
	...
	switch (pose_type)
	{
		...
		case EMOTE_RUNNING:
			if (held)
			{
				attachment_props *att_props = get_attachment_props_if_held(a);
				if (att_props)
					return &att_props->cal_frames[cal_attached_run_frame];
			}
			else
			{
				return &actors_defs[actor_type].cal_frames[cal_actor_run_frame];
			}
		case EMOTE_WALKING:
			if(held)
			{
				attachment_props *att_props = get_attachment_props_if_held(a);
				if (att_props)
					return &att_props->cal_frames[cal_attached_walk_frame];
			}
			else
			{
				return &actors_defs[actor_type].cal_frames[cal_actor_walk_frame];
			}
		default:
			return NULL;
	}
...
static int handle_emote_command(int act_id, emote_command *command)
{
	...
	else if (!act->cur_emote.active)
	{
		...
		// Barehanded?
		if ((command->emote->barehanded & EMOTE_BARE_R)
			&& !is_actor_barehanded(act, EMOTE_BARE_R))
		{
			return 0;
		}
		if ((command->emote->barehanded & EMOTE_BARE_L)
			&&!is_actor_barehanded(act, EMOTE_BARE_L))
		{
			return 0;
		}
		...
		{
			// We have a pose
			hash_entry *he = hash_get(actors_defs[actor_type].emote_frames,
				(NULL+command->emote->anims[act->actor_type][0][held]->ids[0]));
	...
	for (i = 0; i < max_actors; i++)
	{
		if (actors_list[i])
		{
			if (actors_list[i]->calmodel
				...
				&& ACTOR(i)->anim_time >= ACTOR(i)->cur_anim.duration)
			{
				if(HAS_HORSE(i))
				{
					// Rotations during idle animation like when server sends
					// turn_n..turn_nw need to be synchronized on the minimum
					// remaining animation time between the idle of the horse
					// and the actor.
					if (MY_HORSE(i)->cur_anim.kind == cycle)
					{
						MY_HORSE(i)->busy = 0;
			...
			// First thing, decrease the damage time, so we will see the damage
			// splash only for 2 seconds
			...
			// 9 frames, not moving, and another command is queued farther on
			// (based on how long we've done this action)
			...
			if (!ACTOR(i)->busy)
			{
				if (ACTOR(i)->attached_actor >= 0&& ACTOR(i)->actor_id >=0
					&& ((ACTOR(i)->last_command >= enter_aim_mode
							&& ACTOR(i)->last_command <= aim_mode_fire)
						|| (ACTOR(i)->last_command>=enter_combat
							&& ACTOR(i)->last_command <= leave_combat)))
				{
					unfreeze_horse(i);
				}
			}
		...
		if (!actors_list[i]->busy) //Are we busy?
		{
			...
			if (actors_list[i]->cur_emote.active
				&& !(actors_list[i]->que[0] >= move_n
					&& actors_list[i]->que[0] <= move_nw
					&& actors_list[i]->last_command >= move_n
					&& actors_list[i]->last_command <= move_nw)
				&& !HAS_HORSE(i))
			{
				continue;
			}
			...
			{
				if (!handle_emote_command(i, &actors_list[i]->emote_que[0]))
					break;
			}
			...
			else
			{
					...
					&& (actors_list[i]->que[0] < turn_n || actors_list[i]->que[0] > turn_nw))
				{
					...
					LOG_ERROR("%s: %d: command incompatible with range mode detected: %d",
						__FUNCTION__, __LINE__, actors_list[i]->que[0]);
					missiles_log_message("%s (%d): forcing aim mode exit", a->actor_name,
						a->actor_id);
				...
				switch (actors_list[i]->que[0])
				{
					...
					case pain2:
					{
						int painframe = (actors_list[i]->que[0] == pain1)
							? cal_actor_pain1_frame : cal_actor_pain2_frame;
					...
					case leave_combat:
					{
						...
						if (HAS_HORSE(i))
						{
							...
							{
								if (fight_k || ACTOR(i)->horse_rotated)
									rotate_actor_and_horse(i,mul_angle);
								cal_actor_set_anim(i,
							...
							cal_actor_set_anim(MY_HORSE_ID(i),
								actors_defs[MY_HORSE(i)->actor_type].cal_frames[combat_frame]);
						}
						else
						{
							cal_actor_set_anim(i,
					...
					case attack_down_10:
						...
						if (actors_list[i]->is_enhanced_model)
						{
							if (HAS_HORSE(i))
							{
								...
								cal_actor_set_anim(i,
									actors_defs[actor_type].weapon[actors_list[i]->cur_weapon].cal_frames[index]);
							}
							else
							{
						...
						if (HAS_HORSE(i) && !ACTOR(i)->horse_rotated
							&& ACTOR_WEAPON(i)->turn_horse)
						{
							// Horse, not rotated, to be rotated -> do rotation
							rotate_actor_and_horse(i, -1);
						}
						else if (HAS_HORSE(i) && ACTOR(i)->horse_rotated
							&& !ACTOR_WEAPON(i)->turn_horse)
						{
					...
					case turn_right:
					{
						...
						int turnframe = (actors_list[i]->que[0]==turn_left)
							? cal_actor_turn_left_frame : cal_actor_turn_right_frame;
						...
						if (!actors_list[i]->fighting)
						{
							...
							{
								cal_actor_set_anim(i,
									*get_pose_frame(actors_list[i]->actor_type, actors_list[i],
										EMOTE_MOTION(actors_list[i]),1));
							}
							else
							{
								cal_actor_set_anim(i,
				...
				case enter_aim_mode:
					...
					if (actors_list[i]->in_aim_mode == 0)
					{
						missiles_log_message("%s (%d): enter in aim mode",
							actors_list[i]->actor_name, actors_list[i]->actor_id);
						if (actors_list[i]->attached_actor >= 0)
						{
							if (!ACTOR(i)->horse_rotated)
							{
								rotate_actor_and_horse_range(i, -1);
								ACTOR(i)->horse_rotated = 1;
							}
							...
							cal_actor_set_anim(i,
								actors_defs[actor_type].weapon[actors_list[i]->cur_weapon].cal_frames[cal_weapon_range_in_held_frame]);
						}
						else
						{
							cal_actor_set_anim(i,
								actors_defs[actor_type].weapon[actors_list[i]->cur_weapon].cal_frames[cal_weapon_range_in_frame]);
						}
					...
					else
					{
						...
						missiles_log_message("%s (%d): aiming again (time=%d)",
							actors_list[i]->actor_name, actors_list[i]->actor_id, cur_time);
						if (HAS_HORSE(i))
						{
							cal_actor_set_anim(i,
								actors_defs[actor_type].weapon[actors_list[i]->cur_weapon].cal_frames[cal_weapon_range_idle_held_frame]);
						}
						else
						{
							cal_actor_set_anim(i,
								actors_defs[actor_type].weapon[actors_list[i]->cur_weapon].cal_frames[cal_weapon_range_idle_frame]);
						}
						...
						if (action->aim_actor >= 0)
						{
							...
							if (aim_actor)
							{
								cal_get_actor_bone_absolute_position(aim_actor,
									get_actor_bone_id(aim_actor, body_top_bone),
									NULL, action->aim_position);
							}
						}


						if (HAS_HORSE(i))
							ACTOR(i)->z_rot += HORSE_RANGE_ROTATION;
				...
				case aim_mode_fire:
				{
					...
					if (action->reload)
					{
						...
						if (HAS_HORSE(i))
							cal_actor_set_anim(i,
								actors_defs[actor_type].weapon[actors_list[i]->cur_weapon].cal_frames[cal_weapon_range_fire_held_frame]);
						else
							cal_actor_set_anim(i,
								actors_defs[actor_type].weapon[actors_list[i]->cur_weapon].cal_frames[cal_weapon_range_fire_frame]);
						actors_list[i]->in_aim_mode = 1;
					}
					else
					{
						missiles_log_message("%s (%d): fire and leave aim mode",
							actors_list[i]->actor_name, actors_list[i]->actor_id);
						// launch fire and leave aim mode animation

						if (HAS_HORSE(i))
							cal_actor_set_anim(i,
								actors_defs[actor_type].weapon[actors_list[i]->cur_weapon].cal_frames[cal_weapon_range_fire_out_held_frame]);
						else
							cal_actor_set_anim(i,
								actors_defs[actor_type].weapon[actors_list[i]->cur_weapon].cal_frames[cal_weapon_range_fire_out_frame]);
						actors_list[i]->in_aim_mode = 2;
					}
					...
					else if (action->fire_actor >= 0)
					{
						...
						if (fire_actor)
						{
							cal_get_actor_bone_absolute_position(fire_actor,
								get_actor_bone_id(fire_actor, body_top_bone),
								NULL, action->fire_position);
						}
					}

					missiles_fire_arrow(actors_list[i], action->fire_position,
						action->shot_type);
				...
				default:
					if(actors_list[i]->que[0] >= move_n && actors_list[i]->que[0] <= move_nw)
					{
						...
						attachment_props *att_props = get_attachment_props_if_held(actors_list[i]);
						if (att_props)
							walk_anim = get_pose_frame(actors_list[i]->actor_type,
								actors_list[i], EMOTE_MOTION(actors_list[i]), 1);
						else
							walk_anim = get_pose_frame(actors_list[i]->actor_type,
								actors_list[i], EMOTE_MOTION(actors_list[i]), 0);
						...
						if (last_command != actors_list[i]->que[0])
						{
						...
						/* we change the speed of the walking animation according
						 * to the walking speed and to the size of the actor.
						 * We suppose here that the normal speed of the walking
						 * animation is 2 meters per second (1 tile in 250ms)
						 */
					...
					else if(actors_list[i]->que[0] >= turn_n && actors_list[i]->que[0] <= turn_nw)
					{
						...
						int horse_angle = 0;
						if (IS_HORSE(i))
							horse_angle = (ACTOR(i)->fighting && ACTOR_WEAPON(MY_HORSE_ID(i))->turn_horse)
								? HORSE_FIGHT_ROTATION : 0;
						else if (HAS_HORSE(i))
							horse_angle = (ACTOR(i)->fighting&&ACTOR_WEAPON(i)->turn_horse)
								? HORSE_FIGHT_ROTATION : 0;

						if ((HAS_HORSE(i) || IS_HORSE(i))
							&& (ACTOR(i)->in_aim_mode || MY_HORSE(i)->in_aim_mode))
						{
						...
						if (ACTOR(i)->fighting && horse_angle != 0)
							ACTOR(i)->horse_rotated = 1;
						missiles_log_message("%s (%d): rotation %d requested",
							actors_list[i]->actor_name, actors_list[i]->actor_id,
							actors_list[i]->que[0] - turn_n);
					}
				}
				...
				if (!no_action)
					...
					ACTOR(i)->in_aim_mode = 3; // Needed to unfreeze the horse in move_to_next_frame
...
static int push_command_in_actor_queue(unsigned int command, actor *act)
{
	...
	for (k = 0; k < MAX_CMD_QUEUE; k++)
	{
		...
		{
			// If we are SEVERELY behind, just update all the actors in range
			if (k > MAX_CMD_QUEUE-2)
				break;
			if(k > MAX_CMD_QUEUE-8)
			{
...
static void queue_emote(actor *act, emote_data *emote)
{
	...
	if (k > MAX_EMOTE_QUEUE - 2)
	{
		LOG_ERROR("Too many commands in the emote queue for actor %d (%s)",
			act->actor_id, act->actor_name);
	}
}

static void rotate_actor_and_horse_by(int id, int mul, float angle)
{
...
void set_on_idle(int actor_idx)
{
	...
	if (a->fighting)
	{
		if (a->attached_actor >= 0)
		{
			...
			else if (ACTOR_WEAPON(actor_idx)->turn_horse && ACTOR(actor_idx)->horse_rotated)
				cal_actor_set_anim(actor_idx, actors_defs[a->actor_type].cal_frames[cal_actor_combat_idle_held_frame]);
			else
				cal_actor_set_anim(actor_idx, actors_defs[a->actor_type].cal_frames[cal_actor_combat_idle_held_unarmed_frame]);
		}
		else
		{
			cal_actor_set_anim(actor_idx,actors_defs[a->actor_type].cal_frames[cal_actor_combat_idle_frame]);
	...
	else if (a->in_aim_mode == 1)
	{
		if (a->actor_id < 0)
		{
			...
			{
				cal_actor_set_anim(actor_idx,
					*get_pose_frame(a->actor_type, a, EMOTE_STANDING,0));
			}
		}
